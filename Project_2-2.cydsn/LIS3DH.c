...
{
    uint8_t curr;
    ErrorCode err = get_reg(LIS3DH_CTRL_REG1, &curr);
    if(err == ERROR) return err;
    val = (val<<4);
    curr &= 0b00001111;
    val |= curr;
    err = set_reg(LIS3DH_CTRL_REG1, val);
    return err;
}
...
{   
    ...
    uint8_t curr;
    ErrorCode err = get_reg(LIS3DH_CTRL_REG5, &curr);
    if(err == ERROR) return err;
    curr &= 0b10111111;
    val = (val<<6);
    val |= curr;
    err = set_reg(LIS3DH_CTRL_REG5, val);
    if(err == ERROR) return err;
   
    //set FIFO mode
    err = get_reg(FIFO_CTRL_REG, &curr);
    if(err == ERROR) return err;
    curr &= 0b00111111;
    mode = (mode<<6);
    mode |= curr;
    err = set_reg(FIFO_CTRL_REG, mode);
    
    //overrun interrupt enable
    err = get_reg(LIS3DH_CTRL_REG3, &curr);
    if(err == ERROR) return err;
    curr &= 0b11111101;
    val = (val<<1);
    val |= curr;
    err = set_reg(LIS3DH_CTRL_REG3, val);
    return err;    
